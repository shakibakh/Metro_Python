...
// Open Mesh
void OpenMesh(const std::string &filename, CMesh &m)
{
  int err = tri::io::Importer<CMesh>::Open(m,filename.c_str());
  if(err) {
      printf("Error in reading %s: '%s'\n",filename.c_str(),tri::io::Importer<CMesh>::ErrorMsg(err));
  ...
  if(CleaningFlag){
      ...
      printf("Removed %i duplicate and %i unreferenced vertices from mesh %s\n",dup,unref,filename.c_str());
  }
}


void ComputeMetro(const std::string &filename1, const std::string &filename2, bool vertex_sampling, bool edge_sampling, bool face_sampling, bool hist)
{
    printf("Mesh info:\n");
    CMesh                 S1, S2;
    ...
    // load input meshes.
    OpenMesh(filename1,S1);
    OpenMesh(filename2,S2);

    string S1NewName=SaveFileName(filename1);
    string S2NewName=SaveFileName(filename2);
    ...
    // print mesh info.
    printf("Mesh info:\n");
    printf(" M1: '%s'\n\tvertices  %7i\n\tfaces     %7i\n\tarea      %12.4f\n", filename1, S1.vn, S1.fn, ForwardSampling.GetArea());
    ...
    printf(" M2: '%s'\n\tvertices  %7i\n\tfaces     %7i\n\tarea      %12.4f\n", filename2, S2.vn, S2.fn, BackwardSampling.GetArea());
...
PYBIND11_MODULE(metro, m) {
  m.def("ComputeMetro", &ComputeMetro, "ComputeMetro");
}
